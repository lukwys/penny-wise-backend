...
def generate_ai_content(scanned_text: str):
    try:
        response = client.models.generate_content(
            ...
            contents=f'You are a receipt parser. Extract all purchased items and their prices from the following receipt text. Return a JSON array where each element has "name" and "price" (as a number). Return only the JSON, no explanation.\n\nReceipt:\n{scanned_text}',
        )
    except Exception as e:
        raise ParsingError("Cannot parse the receipt")
